...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.getenv("SECRET_KEY", "chave-super-secreta-dev")
...
if database_url:
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = "postgresql://postgres:1234@localhost/catecontrol"
...
def api_dashboard():
    ...
    return jsonify({
        'entradas_hoje': entradas_hoje, 'saidas_hoje': saidas_hoje,
        'presentes': max(0, presentes), 'cadastrados': cadastrados,
        'frequencia': frequencia, 'atividade': [r.to_dict() for r in atividade]
    })
...
def api_atualizar_pessoa(pessoa_id):
    ...
    pessoa.telefone = data.get('telefone', pessoa.telefone) or None
    pessoa.email = data.get('email', pessoa.email) or None
    pessoa.data_nascimento = data.get('data_nascimento', pessoa.data_nascimento) or None
    pessoa.responsavel_codigo = data.get('responsavel_codigo', pessoa.responsavel_codigo) or None
    pessoa.turma = data.get('turma', pessoa.turma) or None
